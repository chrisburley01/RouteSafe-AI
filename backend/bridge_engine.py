...
class RouteLegOut(BaseModel):
    ...
    # extra fields so the frontend can draw a map
    start_lat: float
    start_lon: float
    end_lat: float
    end_lon: float
...
<body>
  ...
  <div class="page">
    <div class="layout">
      ...
      <div class="card">
        <h2>Route map & legs</h2>
        <p class="card-subtitle">Map preview plus each leg checked for low bridges.</p>
  ...
  <script>
    ...
    function ensureMap() {
      ...
      if (!map) {
        ...
        map.setView([53.8, -1.5], 6); // rough UK centre default
      }
      if (!bridgeLayer) {
    ...
    function updateMap(legs) {
      ...
      legs.forEach((leg) => {
        // Straight-line route for now between start and end of each leg
        ...
        (leg.low_bridges || []).forEach((b) => {
          ...
          m.bindPopup(
            `Low bridge ~${b.bridge_height_m.toFixed(2)} m`
          );
